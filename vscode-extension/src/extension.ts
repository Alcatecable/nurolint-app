...
export function activate(context: vscode.ExtensionContext) {
  try {
    ...
    context.subscriptions.push(
      ...
      vscode.commands.registerCommand("neurolint.generateReport", generateReport),
      vscode.commands.registerCommand("neurolint.exportAnalysis", exportAnalysis),
      vscode.commands.registerCommand("neurolint.shareAnalysis", shareAnalysis),
      vscode.commands.registerCommand("neurolint.applySpecificFix", applySpecificFix),
      vscode.commands.registerCommand("neurolint.applyLayerFix", applyLayerFix),
    );
...
async function login(): Promise<void> {
  // NeuroLint is completely free and open source - no authentication required
  vscode.window.showInformationMessage(
    "NeuroLint is completely free and open source! All features (analysis, fixes, and all 7 layers) are available without an API key.",
    "View Documentation"
...
async function generateReport(type?: string): Promise<void> {
  try {
    statusBar.updateStatus("Generating report...", true);
    outputChannel.appendLine(`Generating ${type || 'PDF'} report`);

    const document = vscode.window.activeTextEditor?.document;
    if (!document) {
      vscode.window.showWarningMessage("No active document to generate report for");
      statusBar.updateStatus("Ready");
      return;
    }
    ...
    if (results) {
      ...
      const uri = await vscode.window.showSaveDialog({
        defaultUri: vscode.Uri.file(`neurolint-report.${type === 'pdf' ? 'json' : type || 'json'}`),
        filters: {
          "JSON files": ["json"],
          "All files": ["*"]
        }
    ...
    statusBar.updateStatus("Report Failed");
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    outputChannel.appendLine(`Report generation failed: ${errorMessage}`);
    vscode.window.showErrorMessage(`Report generation failed: ${errorMessage}`);
  }
}

async function exportAnalysis(format?: string): Promise<void> {
  try {
    statusBar.updateStatus("Exporting analysis...", true);
    outputChannel.appendLine(`Exporting analysis in ${format || 'JSON'} format`);

    const document = vscode.window.activeTextEditor?.document;
    if (!document) {
      vscode.window.showWarningMessage("No active document to export analysis for");
      statusBar.updateStatus("Ready");
      return;
    }
    ...
    if (results) {
      ...
      const uri = await vscode.window.showSaveDialog({
        defaultUri: vscode.Uri.file(`neurolint-analysis.${format || 'json'}`),
        filters: {
          "JSON files": ["json"],
          "All files": ["*"]
        }
    ...
    statusBar.updateStatus("Export Failed");
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
...
async function shareAnalysis(): Promise<void> {
  vscode.window.showInformationMessage(
    "NeuroLint is free and open source! Share your feedback at github.com/Alcatecablee/Neurolint",
    "Open GitHub"
  ).then((action: string | undefined) => {
    if (action === "Open GitHub") {
      vscode.env.openExternal(vscode.Uri.parse("https://github.com/Alcatecablee/Neurolint"));
    }
  });
}

async function applySpecificFix(file?: string, fix?: any): Promise<void> {
  try {
    if (!file || !fix) {
      vscode.window.showWarningMessage("No fix specified");
      return;
    }

    statusBar.updateStatus("Applying fix...", true);
    outputChannel.appendLine(`Applying specific fix to ${file}`);
    ...
    if (results) {
      vscode.window.showInformationMessage(`Fix applied successfully to ${file}`);
    }
...
async function applyLayerFix(layerId?: number, fixId?: string): Promise<void> {
  try {
    if (!layerId) {
      vscode.window.showWarningMessage("No layer specified");
      return;
    }

    statusBar.updateStatus("Applying layer fix...", true);
    outputChannel.appendLine(`Applying fix ${fixId || 'all'} from layer ${layerId}`);
    ...
    if (results) {
      vscode.window.showInformationMessage(`Layer ${layerId} fixes applied successfully!`);
    }
    ...
    statusBar.updateStatus("Fix Failed");
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    outputChannel.appendLine(`Layer fix failed: ${errorMessage}`);
    vscode.window.showErrorMessage(`Layer fix failed: ${errorMessage}`);
  }
}
