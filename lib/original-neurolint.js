...
const runOriginalNeuroLint = async (
  ...
  try {
    // Verify fix-master.js exists in CLI directory
    const cliDir = path.join(process.cwd(), 'Neurolint-CLI-main');
    const masterPath = path.join(cliDir, 'fix-master.js');
    if (!fs.existsSync(masterPath)) {
      throw new Error('fix-master.js not found in Neurolint-CLI-main directory');
    }

    // Verify scripts directory exists in CLI directory
    const scriptsDir = path.join(cliDir, 'scripts');
    if (!fs.existsSync(scriptsDir)) {
      throw new Error('scripts/ directory not found in Neurolint-CLI-main');
    }
    ...
    try {
      ...
      // Import from CLI directory (Neurolint-CLI-main/)
      const sharedCore = require('../Neurolint-CLI-main/shared-core');
      ...
      const fixMaster = require('../Neurolint-CLI-main/fix-master');
      ...
      return {
       ...
       try {
         output = execSync(cmd, { 
           encoding: 'utf8',
           cwd: cliDir, // Run in CLI directory so fix-master.js finds its scripts/
