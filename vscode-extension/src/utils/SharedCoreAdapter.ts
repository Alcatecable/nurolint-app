...
export class SharedCoreAdapter implements IAnalysisClient {
   ...
   */
  public async authenticate(_apiKey: string): Promise<UserInfo> {
    // VS Code extension is completely free - all features enabled
    return {
      id: 'vscode-user',
   ...
   * All features are free and open source
   */
  public async canUseFeature(_feature: keyof UserInfo["features"]): Promise<boolean> {
  ...
  public async checkUsageLimit(): Promise<{
    ...
    // No usage limits - everything is free
    return {
      canUse: true,
      usage: { current: 0, limit: -1 },
      tier: 'free'
