...
// Create a service role client for API key validation - lazy loaded to handle missing credentials
...
function getSupabaseService(): SupabaseClient | null {
  ...
  if (!url || !key) {
    console.warn('[API-KEY-UTILS] Supabase credentials not configured - API key validation disabled');
    return null;
  }
...
export const apiKeyService = {
  ...
  async validate(key: string): Promise<ApiKey | null> {
    ...
    // If Supabase isn't configured, API key validation is disabled
    if (!service) {
      console.warn('[API-KEY-UTILS] Cannot validate API key - Supabase not configured');
      return null;
    }
